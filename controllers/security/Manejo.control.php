...
require_once 'models/security/state.model.php';
    ...
    switch($viewData["act"]){
        ...
        case "ACT":
            break;
        default:
            redirectWithMessage("Sucedio un error, vuelve a intentarlo", "index.php?page=Manejos");
    }
    if($_SERVER["REQUEST_METHOD"]=="POST"){
        ...
        if(isset($_POST["btnConfirmar"])){
            ...
            if($varBody["token"]!=$_SESSION["manejos_token"]){
                ...
                redirectWithMessage("Parece que algo salio mal, vuelve a intentarlo", "index.php?page=Manejos");
            }
            if($validated){
                switch($viewData["act"]){
                    case "INS":
